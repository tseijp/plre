...
float ${key}(vec3 p) {
    return ${key}_DE(p, vec3(1. - iMouse.y), 2. - iMouse.x);
}
`
...
float ${key}(vec3 p) {
    return ${key}_DE(p, vec3(1.), 3. - 1. * iMouse.x);
}
`
...
float ${key}(vec3 p) {
  return ${key}_DE(vec4(p - vec3(.5, 0., 0.), 0.));
}
`
...
float ${key}(vec3 p) {
  return ${key}_DE(vec4(p, 0.), vec4(-1., -.2 * (1. - iMouse.x) , 0., 0.));
}
`
